...
| `/Makefile`  | Быстрый и удобный запуск сервиса и автотестов         |
...
### 5.3. Команды для запуска сервиса с помощью Makefile

| Цель                 | Команда (make)   | Описание                                                            |
|----------------------|------------------|---------------------------------------------------------------------|
| Запустить сервис     | `make up`        | Запуск `/launch/docker-compose.yml` + `docker-compose.override.yml` |
| Остановить сервис    | `make down`      | Остановить и удалить конейтнеры                                     |
| Запустить автотесты  | `make test`      | Запуск `/launch/docker-compose.yml` + `docker-compose.test.yml`     |
| Остановить автотесты | `make test-down` | Остановить и удалить автотесты                                      |
...
Автотесты находятся в папкe `/tests/`, для запуска используйте команду `make test`. Отличие от основного решения 
заключается в том, что не запускается `uvicorn`, все тесты работают с помощью:
